...
import { 
    Layers, Globe, ChevronRight, Fuel, Gauge, Lock, Unlock, Loader2, 
    Plus, Minus, Maximize, Crosshair, MapPin, Search, ChevronLeft, Truck
} from 'lucide-react';
...
// Configuração das camadas de mapa (Tile Layers)
const TILE_LAYERS = {
  dark: {
...
export const MapTracker: React.FC<MapTrackerProps> = ({ vehicles, onToggleLock }) => {
  ...
  // Side Panel State - Inicia FECHADO (false) conforme solicitado
  ...
  useEffect(() => {
    ...
    const map = L.map(mapContainerRef.current, { zoomControl: false, attributionControl: false }).setView([BASE_LAT, BASE_LNG], 13);
    tileLayerRef.current = L.tileLayer(TILE_LAYERS[currentMapStyle].url, { attribution: TILE_LAYERS[currentMapStyle].attribution, maxZoom: 19 }).addTo(map);
  ...
  useEffect(() => {
    ...
    vehicles.forEach(vehicle => {
      ...
      if (markersRef.current[vehicle.id]) {
        ...
        if (isSelected) map.flyTo([lat, lng], 15, { duration: 1.5 });
      } else {
        const marker = L.marker([lat, lng], { icon: createCustomIcon(vehicle.status, isSelected, vehicle.isLocked) }).addTo(map);
        marker.on('click', () => { setSelectedVehicleId(vehicle.id); map.flyTo([lat, lng], 15, { duration: 1.5 }); });
  ...
  // Tamanho do menu lateral reduzido para w-72 (aprox 288px)
  const menuWidthClass = 'w-72';
  // Posição do botão de toggle quando aberto (288px + 16px margem = 304px)
  const toggleButtonLeftClass = 'left-[304px]';

  return (
    <div className="relative w-full h-full overflow-hidden bg-slate-950 animate-fade-in flex">
      
      {/* --- SIDE VEHICLE LIST (FLOATING OVERLAY) --- */}
      <div 
        className={`absolute top-4 left-4 bottom-24 md:bottom-4 z-[400] transition-all duration-300 flex flex-col ${
            isListOpen ? menuWidthClass : 'w-0 opacity-0 pointer-events-none'
        }`}
      >
        <div className="bg-slate-950/90 backdrop-blur-xl border border-slate-700/50 rounded-2xl shadow-2xl flex flex-col h-full relative overflow-hidden group">
            {/* Top Gradient Line (MANTIDO CONFORME SOLICITADO) */}
      ...
      <button 
        ...
        className={`absolute z-[400] top-4 transition-all duration-300 bg-slate-950/90 backdrop-blur-xl border border-slate-700/50 text-slate-400 hover:text-white p-2.5 rounded-xl shadow-xl hover:border-blue-500 group ${
            isListOpen ? toggleButtonLeftClass : 'left-4'
        }`}
         ...
         <div className="absolute top-4 right-4 z-[400] flex flex-col items-end gap-2">
             <div className="relative">
                <button 
                    ...
                    className="bg-slate-900/90 backdrop-blur-md text-slate-300 p-3 rounded-xl border border-slate-800 shadow-xl hover:bg-blue-600 hover:text-white transition-all"
                ...
                <div className={`absolute right-0 top-14 bg-slate-900/95 backdrop-blur-xl border border-slate-800 rounded-2xl p-3 shadow-2xl w-48 transform transition-all duration-200 origin-top-right ${showLayerControl ? 'scale-100 opacity-100' : 'scale-95 opacity-0 pointer-events-none'}`}>
                    <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-2">
                        <Globe className="w-3 h-3" /> Estilo
                    </h3>
                    <div className="space-y-1">
                        {(Object.keys(TILE_LAYERS) as MapStyle[]).map((style) => (
                            <button
                                key={style}
                                onClick={() => { setCurrentMapStyle(style); setShowLayerControl(false); }}
                                className={`w-full text-xs p-2 rounded-lg border text-left transition-all ${
         ...
         <div className="absolute bottom-24 md:bottom-8 right-4 z-[400] flex flex-col gap-2">
             <div className="bg-slate-900/90 backdrop-blur-md rounded-xl border border-slate-800 shadow-xl overflow-hidden flex flex-col">
                 <button 
                    ...
                    className={`p-3 transition-colors ${isLocating ? 'text-blue-500 animate-pulse bg-blue-500/10' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
                 ...
                 <button 
                    ...
                    className="p-3 text-slate-400 hover:text-blue-400 hover:bg-slate-800 transition-colors"
                      ...
                      <div className="p-5">
                              ...
                              <div>
                                  ...
                                  <div className="flex items-center gap-2 mt-1">
                                    ...
                                    <span className={`text-[10px] px-2 py-0.5 rounded-full uppercase font-bold tracking-wider ${
                                        selectedVehicleData.isLocked ? 'bg-red-500/20 text-red-400' :
                                        selectedVehicleData.status === VehicleStatus.MOVING 
                                        ? 'bg-green-500/20 text-green-400' 
                                        : 'bg-yellow-500/20 text-yellow-400'
                                    }`}>
                                        {selectedVehicleData.isLocked ? 'BLOQUEADO' : selectedVehicleData.status}
                                    </span>
                                  </div>
                              </div>
                              <button 
                          ...
                          <div className="grid grid-cols-3 gap-3 border-t border-slate-800 py-4">
                              <div className="flex flex-col items-center justify-center p-2 bg-slate-900/50 rounded-lg border border-slate-800">
                                  ...
                                  <span className="text-lg font-bold text-white">{selectedVehicleData.speed}</span>
                                  <span className="text-[10px] text-slate-500 uppercase">km/h</span>
                              </div>
                              <div className="flex flex-col items-center justify-center p-2 bg-slate-900/50 rounded-lg border border-slate-800">
                                  <Fuel className="w-5 h-5 text-purple-500 mb-1" />
                                  <span className="text-lg font-bold text-white">{selectedVehicleData.fuelLevel}%</span>
                                  <span className="text-[10px] text-slate-500 uppercase">Tanque</span>
                              </div>
                              <div className="flex flex-col items-center justify-center p-2 bg-slate-900/50 rounded-lg border border-slate-800">
                                  <MapPin className="w-5 h-5 text-green-500 mb-1" />
                                  <span className="text-lg font-bold text-white">GPS</span>
                                  <span className="text-[10px] text-slate-500 uppercase">Online</span>
                              </div>
                          </div>

                          <button 
                             onClick={() => handleLockClick(selectedVehicleData.id)}
                             disabled={!!loadingLock}
                             className={`w-full py-3 rounded-xl font-bold text-sm shadow-lg flex items-center justify-center gap-2 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed ${
                                 selectedVehicleData.isLocked 
                                 ? 'bg-green-600 hover:bg-green-500 text-white shadow-green-900/20' 
                                 : 'bg-red-600 hover:bg-red-500 text-white shadow-red-900/20'
                             }`}
                          >
                              {loadingLock === selectedVehicleData.id ? (
                                  <Loader2 className="w-4 h-4 animate-spin" />
                              ) : (
                                  selectedVehicleData.isLocked ? <Unlock className="w-4 h-4" /> : <Lock className="w-4 h-4" />
                              )}
